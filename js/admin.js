...
const viewRSVProws = {
  ...
  main: function () {
    $(viewRSVProws.viewOption).change(function () {
      if (this.checked) {
        if (this.value === 'potwierdzona') {
          $(viewRSVProws.tableId).DataTable().search('POTWIERDZONA').draw();
        } 

         else if (this.value === 'anulowana') {
          $(viewRSVProws.tableId).DataTable().search('ANULOWANA').draw();
        } 
        
        else if (this.value === 'oczekująca') {
          $(viewRSVProws.tableId).DataTable().search('OCZEKUJĄCA').draw();
        } else {
          $(viewRSVProws.tableId).DataTable({
            destroy: true,
            select: {style: 'multi'},
